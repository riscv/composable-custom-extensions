...
selected CX. `cxidx` is undefined followed `cxsetsel`.

[NOTE]
====
Alternative proposal: `cxidx` can represent any valid index of the state
context data of _any_ implemented CX. `cxidx` is unchanged following
`cxsetsel`.
====

Writing an invalid index value to `cxidx` may write some other legal
value, which may be a valid legal value.

[NOTE]
====
For example a two bit `cxidx` implementation suffices for a CX with
a four word state context, and such a `cxidx` can only represent four
legal values 0-3, all valid indices.
====
...
When `cxdata` is read (e.g., `csrrs rd,cxdata,x0`), one word of the currently
selected CX state context data, at valid index `cxidx` is copied to the
destination register.

A `cxdata` access by `csrrs` or `csrrc` variant is always a read (not
read-write) operation regardless of the source operand value.

When `cxdata` is written (e.g., `csrrw x0,cxdata,rs1`), the source
register value is written to one word of the currently selected CX state
context data, at valid index `cxidx`.

When `cxdata` is read and written (e.g., `csrrw rd,cxdata,rs1`), both
effects occur.

Following each `cxdata` access, the value `cxidx + 1` is written to
`cxidx`.

Following read/write of the last word of the selected CX's
state context data, the value of `cxidx` is undefined.

When `cxsel` is 0 or invalid, or when `cxidx` is not a valid index of the
selected CX's state context data, read or write of `cxdata` is undefined.
