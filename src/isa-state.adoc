...
The `cxdiscard` instruction is provided to inform the runtime that the
context state data specified by `cxsel` is no longer useful and need
not be saved.  `cxdiscard` raises an illegal instruction exception if
the corresponding `XS` bits are `Off`.  Otherwise, it changes the `XS`
value to `Initial`.

`cxdiscard` raises an illegal instruction exception if `cxsel`
specifies an invalid selection.

[NOTE]
====
`cxdiscard` does not actually write context state data. For ABI
purposes, the context state data is UNSPECIFIED after executing this
instruction.
====
