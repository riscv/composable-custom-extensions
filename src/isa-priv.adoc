...
The privileged CSRs and opcodes are present when `Ztodo` is present
...
When `Zxxx` is not implemented, the bits in `scxxs__n__` for CX IDs
greater than zero are writable and reflect the status of and control
context zero of the respective extension.

When `Zxxx` is implemented, the bits in `scxxs__n__` for CX IDs
